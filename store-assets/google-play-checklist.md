...
Status reviewed 25 August 2026 for release 1.34.0 / version code 52. This is a release
...
- [x] **Fresh signed release bundle:** build version 1.34.0/code 52 with full-state Goa, Madhya Pradesh, Bihar, and Odisha routing;
...
- [ ] Test Karnataka fixtures in Bengaluru, Mysuru, Mangaluru, Hubballi-Dharwad, rural
  districts, the state edge, and neighbouring states. Confirm National Highways run first;
  verified urban-body and Bengaluru routes remain preferred; only the checksum-pinned state
  polygon offers neutral Janaspandana elsewhere; Janahitha is labelled only as an urban
  alternate; and a place name alone never routes. Repeat all three issue types and confirm
  no department, local body, road owner, category acceptance, complaint-write API, automatic
  submission, or statewide contract match is inferred.
- [ ] Test Kerala fixtures in Thiruvananthapuram, Kochi, Kozhikode, Kannur, rural districts,
  the state edge, Mahe, and neighbouring states. Confirm National Highways run first; only
  the checksum-pinned state polygon offers neutral Kerala CMO grievance; K-SMART is labelled
  only as a local-body alternate; the complete GPS-accuracy circle must be inside; and Mahe
  remains excluded. Repeat all three issue types and confirm no department, local body,
  road owner, category acceptance, contract, complaint-write API, or submission is inferred.
- [ ] Test Uttar Pradesh fixtures in Lucknow, Kanpur, Varanasi, Agra, Gorakhpur, Jhansi,
  Saharanpur, Ghaziabad, rural districts, the state edge, Delhi NCT, and neighbouring states.
  Confirm National Highways and exact Delhi routing run first; only the checksum-pinned state
  MultiPolygon offers neutral Jansunwai–Samadhan; the complete GPS-accuracy circle must be
  inside; and a place name alone never routes. Repeat all three issue types and confirm no
  department, local body, road owner, category acceptance, contract, complaint-write API,
  or submission is inferred.
- [ ] Test Chhattisgarh fixtures in Raipur, Bhilai, Bilaspur, Jagdalpur, Ambikapur, Dantewada,
  Korba, rural districts, the state edge, and neighbouring states. Confirm National Highways
  run first; only the checksum-pinned state polygon offers the neutral CM Helpline; NIDAAN
  1100 is labelled only as an urban civic alternate; the complete GPS-accuracy circle must
...
- [ ] Test Goa, Madhya Pradesh, Bihar, and Odisha fixtures in major cities, rural districts,
  neighbouring states, exact boundary edges, and checked-in National Highway vertices.
...
- [ ] Test central, north, south, east, and west Delhi NCT locations, plus Noida,
  ...
  pinned NCT polygon selects the Delhi recipient; Noida and Ghaziabad can route only through
  exact Uttar Pradesh containment, Gurugram stays unrouted, and Faridabad retains its
  independent top-50 match.
...
- [ ] Test statewide Telangana handoffs: Prajavani primary and Citizen Buddy urban alternate
  ...
  types remain user-completed and no department, local body, road owner, category,
  contractor, complaint-write API, automatic submission, or successful filing is claimed.
- [ ] Test statewide Karnataka handoffs: Janaspandana portal and helpline 1902, plus
  Janahitha as an urban alternate, including cancellation and back navigation. Confirm
  verified municipal/Bengaluru routes retain precedence and the neutral fallback never
  claims a department, body, road owner, category, contractor, API submission, or filing.
- [ ] Test statewide Kerala handoffs: CMO grievance portal and help/status line 1076, plus K-SMART
  as a local-body alternate, including cancellation and back navigation. Confirm neither
  route claims a department, body, road owner, category, contractor, API submission, or filing,
  and the app does not claim that 1076 accepts a complaint by phone.
- [ ] Test statewide Uttar Pradesh handoffs: installed and uninstalled Jansunwai package
  `in.nic.up.jansunwai.upjansunwai`, portal fallback, and helpline 1076, including
  cancellation and back navigation. Confirm Delhi keeps its specific route and Jansunwai
  never claims a department, body, road owner, category, contractor, API submission, or filing.
- [ ] Test statewide Chhattisgarh handoffs: CM Helpline OTP portal and helpline 1076, plus
  NIDAAN 1100 only as an urban civic alternate, including cancellation and back navigation.
  Confirm neither route claims a department, body, road owner, category, contractor, API
...
- [ ] Confirm the release contains no API key, test frame, private location, debug-only
  setting, or legacy regional dataset; verify bundled web assets and all 22 resources in the
  v1.34 state-pack manifest against the reviewed byte count and SHA-256. Confirm the old
  unversioned and v1.26–v1.33 manifests remain byte-for-byte immutable.
...
- [ ] Replace or re-verify the icon, feature graphic, and four phone screenshots against
  the 1.34.0 release. Include the issue picker, an independent civic handoff, and regional-language support
